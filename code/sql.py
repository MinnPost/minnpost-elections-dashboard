...
# The exception
SqlError = sqlalchemy.exc.SQLAlchemyError
...
class _State(object):
    ...
    def connection(cls):
        if cls._connection is None:
            ...
            cls.engine = create(DATABASE_URL, echo=cls.echo)
...
def save(unique_keys, data, table_name='swdata'):
    ...
    for row in data:
        ...
        #upsert = insert.values(row).on_conflict_do_update(constraint=unique_keys, set_=data)
        upsert = pg_insert(_State.table).values(row)
...
def get_column_type(column_value):
    ...
    """
    return PYTHON_SQL_TYPE_MAP.get(type(column_value),
